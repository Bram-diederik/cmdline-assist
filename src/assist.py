    ...
    def loop():
        while not should_exit:
            try:
                ...
                if intent_text.startswith("!syscmd"):
                    cmd = intent_text.split(" ")[1].lower()
                    if cmd == "cli":
                        print("Switching to HA CLI...")
                        clean_exit()  # safely exit assist.py for tmux to start ha_cli.py
                    elif cmd == "assist":
                        print("Restarting assist.py...")
                        clean_exit()  # exit; F1/F2 in tmux will restart
                    continue
